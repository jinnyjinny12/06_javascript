...
function outerFuntion(){
    ...
    function innerFunction(){
        ...
        console.log("inner : " , outerVariable); // 내부 함수에서 외부 함수의 변수 접근 가능 (2)
        console.log("inner : " , innerVariable); // 내부 함수에서 내부 변수 접근 가능 (3)
    }
    
    console.log("Outer : " , outerVariable); // 외부 함수에서 외부 변수 접근 가능 (1)
    //console.log("Outer : " , innerVariable); // 오류 발생 

    innerFunction();
}
// 실행순서는 (1) outer (2)inner 중에 먼저 있는 거 (3) inner중에 다음꺼 이런식으로 실행됨!
